...
const findTourByQuery = (query: string): TourPackage | undefined => {
  if (!query) return undefined;
  const q = query.toLowerCase().trim();
  
  // 1. Exact ID or slug match
  let found = TOURS_DATA.find(t => t.id.toLowerCase() === q || t.slug.toLowerCase() === q);
  if (found) return found;

  // 2. Contains slug or query contains slug
  found = TOURS_DATA.find(t => t.slug.toLowerCase().includes(q) || q.includes(t.slug.toLowerCase()));
  if (found) return found;
